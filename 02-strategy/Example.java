...
public final class Example {
  ...
  record ThresholdDiscount(long thresholdCents, long reductionCents) implements DiscountPolicy {
    ThresholdDiscount {
    ...
    public long discount(PricingInput input) {
      return input.subtotalCents() >= thresholdCents
          ? Math.min(reductionCents, input.subtotalCents())
          : 0;
    }
  }
  ...
  static final class PricingService {
    ...
    Quote quote(PricingInput input) {
      ...
      return new Quote(input.subtotalCents(), discount, input.subtotalCents() - discount);
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    check(
        new PricingService(new ThresholdDiscount(0, 500))
                .quote(new PricingInput(200))
                .payableCents()
            == 0);
    check(
        new PercentageDiscount(10000).discount(new PricingInput(Long.MAX_VALUE)) == Long.MAX_VALUE);
    ...
    expect(
        IllegalStateException.class, () -> new PricingService(i -> -1).quote(new PricingInput(10)));
