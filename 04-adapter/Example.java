...
static final class CarrierAdapter implements ShippingGateway {
    ...
    public Booking book(Shipment shipment) throws IOException {
        ...
        var reply = Objects.requireNonNull(client.book(new ProviderRequest(
                shipment.requestId(), shipment.postcode(), grams)),
...
var adapter = new CarrierAdapter(request -> {
    sent.add(request);
...
expect(ArithmeticException.class, () -> adapter.book(new Shipment("x", "560001", Long.MAX_VALUE)));
check(sent.size() == 1);
expect(IOException.class, () -> new CarrierAdapter(r -> new ProviderReply("NEW_STATUS", "x")).book(shipment));
expect(IOException.class, () -> new CarrierAdapter(r -> new ProviderReply("ACCEPTED", "")).book(shipment));
