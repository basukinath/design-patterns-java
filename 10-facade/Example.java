...
interface Catalog { long unitPrice(String sku); }
interface Inventory { boolean available(String sku, int quantity); }
interface Shipping { long fee(Cart cart, long subtotalCents); }
record Preview(long subtotalCents, long shippingCents, long totalCents,
               List<String> unavailableSkus) {
    Preview { unavailableSkus = List.copyOf(unavailableSkus); }
}
...
var facade = new CheckoutFacade(s -> s.equals("sku-a") ? 1000 : 500,
        (s, q) -> s.equals("sku-a"), (c, subtotal) -> 200);
...
expect(IllegalArgumentException.class, () -> new Cart(List.of(new Line("x", 1), new Line("x", 2))));
expect(IllegalStateException.class, () -> new CheckoutFacade(s -> -1, (s,q) -> true, (c,t) -> 0).preview(cart));
expect(ArithmeticException.class, () -> new CheckoutFacade(s -> Long.MAX_VALUE, (s,q) -> true, (c,t) -> 0).preview(cart));
