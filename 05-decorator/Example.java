...
public final class Example {
  ...
  record TimedCatalog(CatalogClient delegate, LongSupplier ticker, Consumer<Observation> sink)
  ...
  record RetryingCatalog(CatalogClient delegate, int maxAttempts) implements CatalogClient {
    RetryingCatalog {
      Objects.requireNonNull(delegate);
  ...
  public static void main(String[] args) throws Exception {
    ...
    long[] time = {0};
    CatalogClient logical =
        new TimedCatalog(new RetryingCatalog(flaky, 2), () -> time[0] += 10, events::add);
    check(logical.fetch("sku-1").cents() == 250);
    check(calls[0] == 2 && events.size() == 1 && events.get(0).success());
    calls[0] = 0;
    events.clear();
    new RetryingCatalog(new TimedCatalog(flaky, () -> time[0] += 10, events::add), 2)
        .fetch("sku-1");
    ...
    expect(
        ...
        () ->
            new RetryingCatalog(
                    s -> {
                      throw new TransientFailure("down");
                    },
                    2)
                .fetch("x"));
