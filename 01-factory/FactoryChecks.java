...
public final class FactoryChecks {
    ...
    public static void main(String[] args) throws Exception {
        var report = new ReportExports.Report(List.of("Order", "Total"),
                List.of(List.of("ORD-1042", "125.00")));
        ...
        ReportExports.PdfRenderer pdf = (actual, options, target) -> {
            check(actual == report && options.equals(pdfOptions) && target == output,
                    "PDF receives report, configured options and caller output");
            calls.add("PDF");
        };
        ReportExports.ExcelRenderer excel = (actual, options, target) -> {
            check(actual == report && options.equals(excelOptions) && target == output,
                    "Excel receives report, configured options and caller output");
            calls.add("EXCEL");
        };
        var factory = new ReportExports.ReportExporterFactory(
                pdf, excel, pdfOptions, excelOptions);
        ...
        check(output.toString(StandardCharsets.UTF_8).equals(
                "\"Order\",\"Total\"\r\n\"ORD-1042\",\"125.00\"\r\n"), "CSV document");
        check(!output.closed, "Caller stream remains open");
        check(factory.create(ReportExports.ExportFormat.PDF)
                != factory.create(ReportExports.ExportFormat.PDF), "Fresh wrapper each call");
        expect(NullPointerException.class, () -> factory.create(null));

        var special = new ReportExports.Report(List.of("Text"), List.of(
                List.of("A, B"), List.of("A \"quote\""), List.of("line\nnext"),
                List.of(""), List.of("Basu — café")));
        var csvOutput = new TrackingOutput();
        new ReportExports.CsvReportExporter().export(special, csvOutput);
        check(csvOutput.toString(StandardCharsets.UTF_8).equals(
                "\"Text\"\r\n\"A, B\"\r\n\"A \"\"quote\"\"\"\r\n"
                + "\"line\nnext\"\r\n\"\"\r\n\"Basu — café\"\r\n"),
        ...
        row.set(0, "changed");
        rows.clear();
        check(snapshot.columns().equals(List.of("Header"))
        ...
        ReportExports.PdfRenderer failingPdf = (r, options, target) -> { throw failure; };
        var failingFactory = new ReportExports.ReportExporterFactory(
                failingPdf, excel, pdfOptions, excelOptions);
        try {
