...
public final class Example {
  abstract static class ImportJob<T> {
    ...
    ImportJob(Consumer<List<T>> sink) {
    ...
    final int run(Reader source) throws IOException {
      ...
      var reader =
          source instanceof BufferedReader buffered ? buffered : new BufferedReader(source);
      ...
      while ((line = reader.readLine()) != null) {
        ...
        try {
          parsed.add(Objects.requireNonNull(parse(line)));
  ...
  static final class StockImport extends ImportJob<StockEntry> {
    StockImport(Consumer<List<StockEntry>> sink) {
      super(sink);
    }
  ...
  static final class PriceImport extends ImportJob<PriceEntry> {
    PriceImport(Consumer<List<PriceEntry>> sink) {
      super(sink);
    }
