...
public final class Example {
  ...
  static final class ExportService {
    ...
    ExportService(AppConfig config) {
      this.config = Objects.requireNonNull(config);
    }

    java.net.URI endpoint() {
      return config.reportEndpoint();
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    var firstConfig =
        new AppConfig(java.net.URI.create("https://a.example"), Duration.ofSeconds(1));
    var secondConfig =
        new AppConfig(java.net.URI.create("https://b.example"), Duration.ofSeconds(2));
    check(
        !new ExportService(firstConfig)
            .endpoint()
            .equals(new ExportService(secondConfig).endpoint()));
    expect(
        IllegalArgumentException.class,
        () -> new AppConfig(java.net.URI.create("/relative"), Duration.ofSeconds(1)));
    expect(
        IllegalArgumentException.class,
        () -> new AppConfig(java.net.URI.create("https://a.example"), Duration.ZERO));
