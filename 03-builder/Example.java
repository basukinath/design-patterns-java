...
enum Format { CSV, PDF }
record ExportRequest(String reportId, Format format, List<String> columns,
                     boolean landscape, boolean includeTotals) {
    ExportRequest {
    ...
    static final class Builder {
        ...
        ExportRequest build() {
            return new ExportRequest(reportId, format, columns,
                    landscape, includeTotals);
        }
    }
}
...
expect(IllegalArgumentException.class, () -> ExportRequest.builder("x", Format.CSV).landscape(true).build());
expect(IllegalArgumentException.class, () -> new ExportRequest("", Format.PDF, List.of("x"), false, false));
