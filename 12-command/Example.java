...
record RenameDocument(UUID commandId, UUID documentId, long expectedVersion,
                      String newTitle) implements Command<Document> {
    RenameDocument {
...
expect(IllegalArgumentException.class, () -> new RenameDocument(command.commandId(), id, 0, "Other").execute(store));
expect(IllegalStateException.class, () -> new RenameDocument(UUID.randomUUID(), id, 0, "Stale").execute(store));
check(store.read(id).title().equals("Final"));
expect(NoSuchElementException.class, () -> new RenameDocument(UUID.randomUUID(), UUID.randomUUID(), 0, "Missing").execute(store));
