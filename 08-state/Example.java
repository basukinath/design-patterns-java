...
enum Status { NEW, AUTHORIZED, CAPTURED, CANCELLED }
interface Phase {
    Status status();
    default Phase authorize() { throw new IllegalStateException("Cannot authorize " + status()); }
    default Phase capture() { throw new IllegalStateException("Cannot capture " + status()); }
    default Phase cancel() { throw new IllegalStateException("Cannot cancel " + status()); }
}
enum NewPayment implements Phase {
    INSTANCE;
    public Status status() { return Status.NEW; }
    public Phase authorize() { return Authorized.INSTANCE; }
    public Phase cancel() { return Cancelled.INSTANCE; }
}
enum Authorized implements Phase {
    INSTANCE;
    public Status status() { return Status.AUTHORIZED; }
    public Phase capture() { return Captured.INSTANCE; }
    public Phase cancel() { return Cancelled.INSTANCE; }
}
...
enum Cancelled implements Phase {
    INSTANCE;
    public Status status() { return Status.CANCELLED; }
}
static final class Payment {
    private Phase phase;
    Payment(Status persistedStatus) {
        phase = switch (Objects.requireNonNull(persistedStatus)) {
    ...
    Status status() { return phase.status(); }
    void authorize() { phase = phase.authorize(); }
    void capture() { phase = phase.capture(); }
    void cancel() { phase = phase.cancel(); }
}
