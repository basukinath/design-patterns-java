...
enum NotificationOutcome { SENT, FAILED }
...
static final class ReviewCoordinator {
    ...
    ReviewCoordinator(ReviewerDirectory directory, ReviewStore store,
...
static final class MemoryReviews implements ReviewStore {
    ...
    synchronized int size() { return reviews.size(); }
}
...
check(sent.size() == 2);
var unavailable = new ReviewCoordinator(s -> reviewer, reviews, r -> { throw new IOException("Offline"); });
check(unavailable.submit(submission).notification() == NotificationOutcome.FAILED);
check(reviews.size() == 1);
expect(IllegalArgumentException.class, () -> new ReviewCoordinator(s -> author, reviews, sent::add).submit(submission));
expect(IllegalArgumentException.class, () -> coordinator.submit(new Submission(submission.documentId(), UUID.randomUUID())));
