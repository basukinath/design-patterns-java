...
public final class Example {
  ...
  static RiskCheck highValue(long thresholdCents) {
    ...
    return input ->
        input.totalCents() >= thresholdCents
  ...
  public static void main(String[] args) throws Exception {
    ...
    check(
        stopped.evaluate(new RiskInput(true, 10)).orElseThrow().ruleId().equals("blocked-account"));
    check(laterCalls[0] == 0);
    expect(IllegalArgumentException.class, () -> new RiskChain(List.of()));
    expect(
        NullPointerException.class,
        () -> new RiskChain(List.of(i -> null)).decide(new RiskInput(false, 1)));
